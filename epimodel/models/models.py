...
def ppool_model(
    ...
    **kwargs,
    ...
    alpha_ic_noise = numpyro.sample(
        "alpha_ic_noise", dist.Normal(scale=0.1 * jnp.ones((data.nCs, data.nCMs)))
    )
    ppool_cm_scale = (ppool_total_scale ** 2 / data.nCMs) ** 0.5
    alpha_ic_delta = ppool_cm_scale * 10.0 * alpha_ic_noise
    alpha_ic = numpyro.deterministic(
        "alpha_ic", alpha_i.reshape((1, data.nCMs)) + alpha_ic_delta
    )
    alpha_ir = data.RC_mat @ alpha_ic

    # transmission reduction. sum over axis 1 (cm axis)
    cm_reduction = jnp.sum(
        data.active_cms * alpha_ir.reshape((data.nRs, data.nCMs, 1)), axis=1
    )
...
def var_ppool_model(
    ...
    **kwargs,
    ...
    ppool_total_scale = numpyro.sample(
        "ppool_total_scale", dist.HalfNormal(ppool_total_scale_prior)
    )
    ppool_cm_scale = (ppool_total_scale ** 2 / data.nCMs) ** 0.5
    alpha_ic_delta = ppool_cm_scale * 10.0 * alpha_ic_noise
    alpha_ic = numpyro.deterministic(
...
def default_model_shared_output_scale(
    data,
    ep,
    intervention_prior=None,
    basic_R_prior=None,
    ...
    **kwargs,
    ...
    #  shared output noise scales
    psi_cases = numpyro.sample(
        "psi_cases",
        dist.HalfNormal(scale=output_noise_scale_prior),
    )
    psi_deaths = numpyro.sample(
        "psi_deaths",
        dist.HalfNormal(scale=output_noise_scale_prior),
    ...
    # use the per country psi_cases and psi_deaths and form a nRs x nDs array
    # to use for the output distribution.
    cases_conc = jnp.ones((data.nRs, data.nDs)) * psi_cases

    deaths_conc = jnp.ones((data.nRs, data.nDs)) * psi_deaths
...
def default_model_country_rw(
    ...
    **kwargs,
    ...
    cs_r_walk_noise_scale = numpyro.sample(
        "cs_r_walk_noise_scale", dist.HalfNormal(scale=r_walk_noise_scale_prior)
    )

    # rescaling variables by 10 for better NUTS adaptation
    cs_r_walk_noise = numpyro.sample(
        "cs_r_walk_noise",
        dist.Normal(loc=jnp.zeros((data.nCs, nNP)), scale=1.0 / 10),
    )

    # only apply the noise every "cs_r_walk_period" - to get full noise, repeat
    expanded_cs_r_walk_noise = jnp.repeat(
        cs_r_walk_noise_scale * 10.0 * jnp.cumsum(cs_r_walk_noise, axis=-1),
        r_walk_period,
        axis=-1,
    )[: data.nCs, : (data.nDs - 2 * r_walk_period)]
    # except that we assume no noise for the first 3 weeks
    cs_full_log_Rt_noise = jnp.zeros((data.nCs, data.nDs))
    cs_full_log_Rt_noise = data.RC_mat @ jax.ops.index_update(
        cs_full_log_Rt_noise, jax.ops.index[:, 2 * r_walk_period:], expanded_cs_r_walk_noise
    )

    Rt = numpyro.deterministic(
        "Rt",
        jnp.exp(
            jnp.log(basic_R.reshape((data.nRs, 1))) + full_log_Rt_noise - cm_reduction + cs_full_log_Rt_noise
        ),
    )
    ...
    numpyro.deterministic("Rt_cs_walk", jnp.exp(cs_full_log_Rt_noise))
