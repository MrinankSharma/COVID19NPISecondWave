...
def candidate_model_v8_ifr_reparam(
    data,
    ep,
    iar_noisescale_prior=None,
    r_walk_noise_scale_period=7,
    ...
    **kwargs
    ...
    ifr_0_tilde = numpyro.sample(
        "ifr_0_tilde", dist.Normal(loc=jnp.zeros((data.nCs, 1)), scale=1)
    )
    ifr_0 = numpyro.deterministic(
        "ifr_0", jnp.clip(0.1 + (0.05 * ifr_0_tilde), a_min=1e-3, a_max=1)
    )
