...
def candidate_model_v12_hypothesis_v3_rescale(
    ...
    **kwargs
    ...
    alpha_i_tilde = numpyro.sample(
        "alpha_i_tilde",
        AsymmetricLaplace(asymmetry=0.5 * jnp.ones(data.nCMs), scale=40 / 2.0),
    )
    alpha_i = numpyro.deterministic("alpha_i", alpha_i_tilde / 2.0)
...
def candidate_model_v12_hypothesis_v3_altrescale(
    data,
    ep,
    iar_noisescale_prior=None,
    r_walk_noise_scale_period=7,
...
def candidate_model_v12_hypothesis_v3_less_npis(
    ...
    **kwargs
    ...
    cm_reduction = jnp.sum(
        data.active_cms[:, :12, :] * alpha_i.reshape((1, 12, 1)), axis=1
    )
