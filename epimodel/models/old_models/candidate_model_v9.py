...
def candidate_model_v9(
    ...
    **kwargs
    ...
    r_walk_noise_scale_tilde = create_noisescale_prior(
        "r_walk_noise_scale_tilde", {"type": "half_normal", "scale": 1}, type="r_walk"
    )
    ...
    iar_noise_scale_tilde = numpyro.sample("iar_noise_scale_tilde", dist.HalfNormal(1))
...
def candidate_model_alt_seeding(
    data,
    ep,
    iar_noisescale_prior=None,
    r_walk_noise_scale_period=7,
    ...
    **kwargs
    ...
    # exponential seeding
    seeding_scale = numpyro.sample(
        "seeding_scale", dist.Exponential(0.03 * jnp.ones((data.nRs, 1)))
    )
    seeding = numpyro.sample("seeding", dist.Exponential(1 / seeding_scale))
    init_infections = jnp.zeros((data.nRs, total_padding))
    init_infections = jax.ops.index_add(
        init_infections,
        jax.ops.index[:, -seeding_padding:],
