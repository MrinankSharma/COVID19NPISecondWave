...
def candidate_model_v13(
    ...
    infection_noise_scale = numpyro.sample("infection_noise_scale", dist.HalfNormal(1))
...
def candidate_model_v13b(
    ...
    scaled_inf_noise = infection_noise_scale * infection_noise

    def discrete_renewal_transition(infections, R_noise_tuple):
        R, infection_noise = R_noise_tuple

        # enforce that infections remain positive with a softplus
        mean_new_infections_t = jax.nn.softplus(
            jnp.multiply(R, infections @ ep.GI_flat_rev) + (10 * infection_noise)
        )
        new_infections = infections
        new_infections = jax.ops.index_update(
            new_infections, jax.ops.index[:, :-1], infections[:, 1:]
